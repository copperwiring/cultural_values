...
def main(user_choice=False):
    ...
    if user_choice:
        use_images = input("Do you want to use images? (Yes/No): ").strip().lower() in ['yes', 'y']
        use_country_name = input("Do you want to use country name in the prompt? (Yes/No): ").strip().lower() in ['yes', 'y']

        processor = LLAVAProcessor(family_data['imageFullPath'].tolist(), questions[:def_n_questions], labeled_options)
        ...
        for use_images in [True, False]:
            for use_country_name in [True, False]:
                print("*"*60)
...
if __name__ == "__main__":
    ...
    main(user_choice=True)  # Modify as needed for automated (False) or manual (True) execution
