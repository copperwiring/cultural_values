...
if __name__ == "__main__":
    ...
    data_dir = "/home/vsl333/cultural_values/csv_inference_dataset"
    out_dir = f"/home/vsl333/cultural_values/gpt_{api_model}_csv_data"
    if os.path.exists(out_dir):
        shutil.rmtree(out_dir)
    os.makedirs(out_dir, exist_ok=True)
    print(f"Output directory created: {out_dir}")
    print("Model used: ", api_model)
    ...
    for each_csv_file in tqdm(csvfile_list):
        df = pd.read_csv(os.path.join(data_dir, each_csv_file))
        outpt_file_path = os.path.join(out_dir, f"{each_csv_file.split('.')[0]}_gpt_output.csv")
        if os.path.exists(outpt_file_path):
            os.remove(outpt_file_path)

        # df = df[df['category']=='Traditions / art / history'] # Filter the data based on category
        # breakpoint()
        print(f"Processing {each_csv_file} file")
        ...
        for row_id, row in df.iterrows():
            ...
            if all(value == 0 for _, value in top_k):
                ...
                top_k_prob = [(option, 0) for option in normalized_option_list]
            else:
                ...
                exp_top_k = [(option, math.exp(value)) for option, value in top_k]
                sum_exp_topk = sum([value for _, value in exp_top_k])
                top_k_prob = [(option, value / sum_exp_topk) for option, value in exp_top_k]
    
            top_k_prob_percent = [(option, round(value*100, 2)) for option, value in top_k_prob]
            # Sort based on alphabetical order
            top_k_pv_values = [value for _, value in top_k_prob_percent]
            ...
            df.at[row_id, 'top_k_pv'] = top_k_pv_values

            print(f"Row {row_id} processed")

        # write to csv file
        with open(outpt_file_path, 'a') as f:
            df.to_csv(f, index=False, header=True)
            print(f"File saved: {outpt_file_path}")

    # # Save the dataframe with the new columns
    # df.to_csv(os.path.join(out_dir, f"{each_csv_file}_gpt_output.csv"), index=False)
