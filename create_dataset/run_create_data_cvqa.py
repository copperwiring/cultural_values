...
def main() -> None:
    ...
    df = df[~df['category'].isin(['plants_and_animals', 'vehicle_and_transportation'])]
    # exclude Iran
    ...
    cvqa_country_data.loc[:, 'image_full_path'] = cvqa_country_data['image_path'].apply(lambda x: Path(cvqa_data_folder) / x)
