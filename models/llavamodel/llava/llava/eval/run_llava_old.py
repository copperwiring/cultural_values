...
def eval_model(args, prompts_batch, img_files_batch=None, letter_options=None, full_options=None, tokenizer=None, model=None, image_processor=None, model_name=None):
    ...
    for i, (prompt, letter_option) in enumerate(zip(batched_prompts, letter_options)):
        ...
        if probs_for_instance.dim() > 0:
            top_k = min(20, probs_for_instance.size(0))  # Ensure k is within the range of available tokens
        else:
            top_k = 0  # Handle case where there are no dimensions or valid tokens
        ...
        if top_k > 0:
            ...
            # Create top10_token_prob as a list of tuples, filtering only option labels
            top10_token_prob = [
                (token, prob.item()) 
                for token, prob in zip(top_10tokens, top_10probs)
                if token in letter_options
            ]
            
    
        # If no option labels are in top10_token_prob, set it to an empty list
        if not top10_token_prob:
            top10_token_prob = []
        ...
        if top10_token_prob != 0:
            ...
            for i in range(65, 65+len_letter_option):
                if chr(i) not in prob_percent.keys():
                    prob_percent[chr(i)] = 0.0
                    
            # Ensure prob_percent is of length len_letter_option by removing extra letters
            if len(prob_percent) > len_letter_option:
                prob_percent = dict(list(prob_percent.items())[:len_letter_option])
